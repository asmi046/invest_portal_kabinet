...
>
    ...
    <div class="columns-box columns-box--two-col">
        <x-form.input name="applicant_name" label="Организация или Ф.И.О. заявителя" :required="false" value="{{ $item->applicant_name ?? old('applicant_name') }}" />
        <x-form.input name="address" label="Адрес" :required="false" value="{{ $item->address ?? old('address') }}" />

        <x-form.input name="phone" label="Телефон" :required="false" value="{{ $item->phone ?? old('phone') }}" />

        <x-form.input name="email" label="Адрес электронной почты" :required="false" value="{{ $item->email ?? old('email') }}" />
    </div>
    ...
    <div class="columns-box columns-box--two-col">
        ...
        <x-form.input name="object_name" label="Адрес объекта" :required="false" value="{{ $item->object_address ?? old('object_address') }}" />
    </div>
    <x-form.textarea name="connection_reason" label="Краткая характеристика объекта" :required="false" value="{{ $item->object_description ?? old('object_description') }}" />
